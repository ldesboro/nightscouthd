...
var refresh_rate = 5000; // initial scan, 5 seconds

var simulated = false;

var now = (simulated === true ? new Date("2013-10-17T07:56:00.123Z").getTime() : new Date().getTime());
...
function update() {
    ...
    if (initialize === true) {
        now = (simulated === true ? new Date("2013-10-17T07:56:00.123Z").getTime() : new Date().getTime());
    }
    else {
        now = (simulated === true ? now + 5 * 60 * 1000 : Date.now());
    }
	refresh_rate = (simulated === true ? 0.02 : 1) * 60 * 1000 ;
...
io.sockets.on('connection', function (socket) {
	//socket.emit("treatment", treatment);
